...
function patchNetHandlerPlayClientHandleCollectItem(method, obfuscated) {
    ...
    for (var i = 0; i < length; i++) {
        ...
        if (node instanceof MethodInsnNode && node.getOpcode().equals(Opcodes.INVOKEVIRTUAL) && node.owner.equals("net/minecraft/network/play/server/SPacketCollectItem") && node.name.equals(getAmount) && node.desc.equals("()I")) {
    ...
    if (foundNode != null) {
        ...
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "com/fuzs/pickupnotifier/handler/AddEntriesHandler", "onEntityPickup", "(Lnet/minecraft/entity/Entity;Lnet/minecraft/entity/EntityLivingBase;)V", false));
